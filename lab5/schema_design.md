...
|----------|------|-------------------|
| `users` | PostgreSQL | Жёсткая схема, уникальный логин (`UNIQUE`), bcrypt-хэш через `pgcrypto`, поиск по триграммам (`pg_trgm` + ILIKE), редкие изменения структуры |
...
- у одного пользователя могут быть тысячи записей бюджета — embedding раздул бы документ users в Postgres;
- доходы/расходы читаются и обновляются независимо от профиля пользователя;
- запрос «динамика бюджета за период» эффективно покрывается составным индексом `{ user_id, date }`.
