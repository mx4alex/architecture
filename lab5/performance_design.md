...
| `GET /api/v1/budget/dynamics` | чтение + агрегация | помесячная/посуточная динамика | высокая (дашборды) |
| `GET /api/v1/users/search` | чтение | поиск по логину или маске | высокая |
| `POST /api/v1/auth/login` | запись (issue JWT) | проверка пароля | средняя |
...
### 1.2 Потенциально медленные операции
...
### 1.3 Требования к производительности (SLO)
...
### 3.1 Какие endpoint-ы ограничиваем
