...
USERVER_CONFIG_HOOKS = ['userver_redis_config']
...
@pytest.fixture(scope='session')
def userver_redis_config():
    def _patch_config(config_yaml, config_vars):
        components = config_yaml['components_manager']['components']
        redis_component = components.get('budget-cache-redis')
        if redis_component is not None:
            for group in redis_component.get('groups', []):
                group.pop('sharding_strategy', None)
