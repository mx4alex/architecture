...
std::string RegisterHandler::HandleRequestThrow(
    ...
    const auto ip = GetClientIp(request);
    const auto decision = rate_limiter_.CheckFixedWindow(
        "rl:register:" + ip, rate_limiter_.Register(), "register");
    ...
    cache_.BumpVersion(std::string{kUsersSearchVersionKey});
...
std::string LoginHandler::HandleRequestThrow(
    ...
    const auto ip = GetClientIp(request);
    const auto decision = rate_limiter_.CheckSlidingWindow(
        "rl:login:" + ip, rate_limiter_.Login(), "login");
    ApplyRateLimitHeaders(response, decision);
    if (!decision.allowed) {
        response.SetStatus(userver::server::http::HttpStatus::kTooManyRequests);
        return R"({"error":"Too many requests, please retry later"})";
