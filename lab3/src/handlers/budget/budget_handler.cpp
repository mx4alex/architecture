...
std::string IncomeHandler::HandleRequestThrow(
    ...
    if (request.GetMethod() == userver::server::http::HttpMethod::kPost) {
        ...
        auto income = storage_.CreateIncome(
            user_id, category, DoubleToDecimal(amount_d),
            userver::utils::datetime::DateFromRFC3339String(date_str),
            description);
...
std::string IncomeByIdHandler::HandleRequestThrow(
    ...
    if (request.GetMethod() == userver::server::http::HttpMethod::kPut) {
        ...
        auto updated = storage_.UpdateIncome(
            user_id, income_id, category, DoubleToDecimal(amount_d),
            userver::utils::datetime::DateFromRFC3339String(date_str),
            description);
        if (!updated) {
...
std::string ExpenseHandler::HandleRequestThrow(
    ...
    if (request.GetMethod() == userver::server::http::HttpMethod::kPost) {
        ...
        auto expense = storage_.CreateExpense(
            user_id, category, DoubleToDecimal(amount_d),
            userver::utils::datetime::DateFromRFC3339String(date_str),
            description);
        response.SetStatus(userver::server::http::HttpStatus::kCreated);
...
std::string ExpenseByIdHandler::HandleRequestThrow(
    ...
    if (request.GetMethod() == userver::server::http::HttpMethod::kPut) {
        ...
        auto updated = storage_.UpdateExpense(
            user_id, expense_id, category, DoubleToDecimal(amount_d),
            userver::utils::datetime::DateFromRFC3339String(date_str),
            description);
        if (!updated) {
            response.SetStatus(userver::server::http::HttpStatus::kNotFound);
...
std::string BudgetDynamicsHandler::HandleRequestThrow(
    ...
    auto from = userver::utils::datetime::DateFromRFC3339String(from_str);
    auto to = userver::utils::datetime::DateFromRFC3339String(to_str);
    ...
    builder["period_from"] = userver::utils::datetime::ToString(dynamics.period_from);
