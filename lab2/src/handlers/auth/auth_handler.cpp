...
AuthCheckerBearer::AuthCheckResult AuthCheckerBearer::CheckAuth(
    ...
    auto user_id = VerifyJwt(token, jwt_secret_);
    if (!user_id) {
...
std::string RegisterHandler::HandleRequestThrow(
    ...
    auto user = storage_.CreateUser(login, password, first_name, last_name);
    if (!user) {
    ...
    auto dto = UserResponse::FromUser(*user);
...
std::string LoginHandler::HandleRequestThrow(
    ...
    auto token = CreateJwt(user->id, storage_.GetJwtSecret());
